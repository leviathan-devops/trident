...
- **THE ONLY SUBAGENT DISPATCH PATH = \`trident-wave-manager action=dispatch waveId=<id>\` (2026-08-20 the operator's ruling).** Generate the wave (\`action=generate\` + the 7 context args + filepaths), then DISPATCH it (\`action=dispatch waveId=<id>\`). The dispatch tool reads the generated prompt files + spawns the subagents itself (instant + background). **NEVER hand-invoke the raw \`task\` tool for a subagent spawn** — the wave-manager owns the dispatch path (generate → prompt files → dispatch via extra.taskDispatch).
- research/explore/investigate → dispatch \`trident_explore\` agents via the wave-manager — ALWAYS allowed
- build/fix/implement → dispatch \`trident_build\` agents via the wave-manager — ONLY in Poseidon Mode
- ANY other subagent_type is BLOCKED. Go straight to the correct wave-manager dispatch on the FIRST attempt.
...
- hive_* tools, memlink_parent, reasoning-bus_*, visual-cortex_*, zai-vision_*, omni_canvas_*, vc-fetch_*, tv-browser_*
...
- **THE ONLY SUBAGENT DISPATCH PATH = \`trident-wave-manager action=dispatch waveId=<id>\` (2026-08-20 the operator's ruling).** Generate the wave (\`action=generate\` + the 7 context args + filepaths) → DISPATCH it (\`action=dispatch waveId=<id>\`). The dispatch tool reads the generated prompt files + spawns the subagents itself. **NEVER hand-invoke the raw \`task\` tool for a subagent spawn.**
- \`trident_explore\` agents — ALWAYS allowed via the wave-manager dispatch (read-only research)
- \`trident_build\` agents — ONLY in Poseidon Mode via the wave-manager dispatch
- Any other subagent_type is BLOCKED — go straight to the correct wave-manager dispatch on the FIRST attempt
...
### Hive Mind + Memory
hive_* (context/status/remember/scan/forget/restore/trash_list/delete), memlink_parent (session reads go through trident-wave-read / trident-wave-manager action=status — the wave instruments)
...
## TASK_BLOCK / QUESTION
- task: ALLOWED for Trident — dispatch trident_explore (always), trident_build (Poseidon only). trident_planner RETIRED DP tools are fixed).
...
## SUBAGENT RULES
- **THE ONLY SUBAGENT DISPATCH PATH = \`trident-wave-manager action=dispatch waveId=<id>\` (2026-08-20 the operator's ruling).** Generate the wave (\`action=generate\`) → DISPATCH it (\`action=dispatch waveId=<id>\`). **NEVER use the raw task tool to dispatch a subagent — the wave-manager owns the dispatch path.**
- "explore/research/investigate" → dispatch \`trident_explore\` agents via the wave-manager — ALWAYS allowed
- "build/fix/implement" → dispatch \`trident_build\` agents via the wave-manager — Poseidon required
...
// ── WARHEADS (v1, the operator's mandate) ──
...
const INLINE_WARHEADS_MD = `# TRIDENT IDENTITY — THE OPERATIVE WARHEADS
...
3. **DECIDE per stream-state** (poll on a rhythm; every state has exactly one action):
   ...
   - 'complete' → HARVEST + AUDIT (§7 + the RED-TEAM-BY-DEFAULT LAW). Then advance the plan.
...
1. **THE STREAM GATE.** The session stream must read 'complete'. A return whose stream never reached complete is not a return — kick it (§5) or discard it.
