  ...
  try {
    ...
    for (const kbId of kbIds) {
      try {
        ...
        // Best effort — individual KB pre-warm is non-critical
      }
    }
    tridentLog('INFO', 'warhead-synthesizer', `P2-B: knowledge-loader cache pre-warmed with ${kbIds.length} KB IDs`);
  } catch (e: unknown) {
    tridentLog('INFO', 'warhead-synthesizer', `P2-B: knowledge-loader pre-warm unavailable: ${e instanceof Error ? e.message : String(e)}`);
  }
