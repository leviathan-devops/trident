...
function generateReasoningChain(
  ...
  return steps.map((step: ReasoningStep) => {
    ...
    const matchedPatterns = discovery.patterns.filter((p: DiscoveredPattern) => {
      // Match by file path
      if (fileRefs.some((ref: string) => p.file.includes(ref.replace(/:\d+$/, '')))) return true;
      // Match by pattern name containing an identifier from the reasoning
      if (identifiers.some((id: string) => p.name.toLowerCase().includes(id.toLowerCase()))) return true;
      return false;
    });
...
function generateRCA(
  ...
  const scored = steps.map((step: ReasoningStep, i: number) => {
    let score = 0;
  ...
  scored.sort((a: ScoredStep, b: ScoredStep) => b.score - a.score);
  ...
  const stepsWithRealEvidence = steps.filter((s: ReasoningStep) =>
...
export function generatePlanArtifact(
  ...
  a += `| **Confidence** | ${rca.confidence} (${steps.filter((s: ReasoningStep) => s.evidence.startsWith('Found:')).length}/${steps.length} steps have code evidence) |\n`;
  a += `\n`;
  ...
  if (findingsLog.length > 0) {
    ...
    a += `|---|---------|----------|--------|\n`;
    findingsLog.forEach((f: { finding: string; severity: string; source: string }, i: number) => {
  ...
  a += `\n---\n*Generated by Trident v4.4.2 Problem-Solving Engine*\n`;
