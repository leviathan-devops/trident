...
export function testZeroTolerance(): number {
  ...
  fc.assert(fc.property(fc.constantFrom('INVALID', '', '{}', '[]', '  '), (evt) => {
    ...
    try { const s = interpret(orchestratorMachine).start(); s.send({ type: evt }); } catch { ok = false; }
    return ok;
  }), { numRuns: 100 }); c += 100;
  ...
  fc.assert(fc.property(fc.array(fc.anything()), (arr) => {
    try { deduplicateFindings(arr as any); return true; } catch { return false; }
  }), { numRuns: 50 }); c += 50;
...
export function testInvariants(): number {
  let c = 0;
  fc.assert(fc.property(fc.array(fc.record({ file: fc.string(), category: fc.string() })), (items) => {
    const r = deduplicateFindings(items.map((x, i) => ({ ...x, line: i, severity: 'HIGH' as const, layer: 0, detector: 'd', title: 't', evidence: 'e', remediation: 'r', evidenceType: 'STATIC' as const })));
  ...
  fc.assert(fc.property(fc.string({ minLength: 1 }), (id) => {
    ...
    return typeof s.getSnapshot().value === 'string' || typeof s.getSnapshot().value === 'object';
  }), { numRuns: 50 }); c += 50;
  return c;
}
...
export function testFuzz(): number {
  ...
  fc.assert(fc.property(fc.array(fc.string({ minLength: 1, maxLength: 10 }), { minLength: 1, maxLength: 20 }), (ids) => {
    try {
      const s = interpret(orchestratorMachine).start();
      for (const id of ids) { s.send({ type: 'START_SESSION', sessionId: id }); s.send({ type: 'END_SESSION', sessionId: id }); }
      return s.getSnapshot().value === 'idle';
    } catch { return false; }
  }), { numRuns: 100 }); c += 100;
  ...
  fc.assert(fc.property(fc.array(fc.oneof(fc.constant(null), fc.constant(undefined), fc.constant({}))), (items) => {
    try { const r = deduplicateFindings(items as any); return Array.isArray(r); }
    catch { return false; }
  }), { numRuns: 50 }); c += 50;
  return c;
}
