...
var BLOCKED_TOOLS_FOR_TRIDENT = [
  ...
  'spawn_shark_agent', 'spawn-shark-agent', 'spawn_manta_agent', 'spawn-manta-agent', 'run_parallel_tasks',
];
...
async function checkTheatricalPatterns(toolName: string, input: Record<string, unknown>): Promise<{ blocked: boolean; category?: string; reason?: string }> {
  ...
  if (!allArgsString) return { blocked: false }; // Monitor: passes by default when rule doesn't apply
  ...
  return { blocked: false }; // Monitor: passes by default when rule doesn't apply
}
...
var chatMessageHook = async function(input: Record<string, unknown>, output: Record<string, unknown>) {
  ...
  var sid = (input as InputMessage)?.sessionID;
  ...
  if (isUserInput) {
    ...
    // NLP Pipeline: process user message for intent routing
    if (outputText) {
...
var toolBeforeHook = async function(input: Record<string, unknown>, output: Record<string, unknown>) {
  ...
  try {
    ...
    checkIdentityBeforeTool(idAgent, idTool, sessionId);
  ...
  if (idTool === 'task') {
    ...
    // TASK SUBAGENT GATE: Only trident_explore subagent allowed for task tool
    if (toolName === 'task' && !isExploreTask) {
      ...
      throw new Error('[TRIDENT TOOL BLOCK] task: only trident_explore subagent allowed. trident_explore is Trident\'s subagent type.');
    }
  }
...
var systemTransformHook = async function(input: Record<string, unknown>, output: Record<string, unknown>) {
  ...
  systemOut.system.push('[TRIDENT v4.3.3] When asked "who are you" or "what are you", '
  ...
  var contextLines = [
    '[TRIDENT v4.3.3] Agent: ' + (sessionAgent || '') + ' | Mode: ' + state.mode + ' | Layer: ' + state.currentLayer + '/17 | Status: ' + state.status,
    '[TRIDENT v4.3.3] CORE PRINCIPLE: "Trident Audits & Generates Review Artifacts. Build Agents Implement All Changes."',
...
var messagesTransformHook = async function(
  ...
  try {
    ...
    if (currentSystem.indexOf('TRIDENT v4.3.3 IDENTITY BINDING') !== -1) return;
