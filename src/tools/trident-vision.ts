 ...
 *   TRIDENT_VLM_ENDPOINT - llama-server endpoint (default: http://127.0.0.1:8082)
 *   TRIDENT_VLM_MODEL - model name sent to API (default: GLM-4.6V-Flash-Q4_K_M.gguf)
 */

const SUPPORTED_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'];
const VLM_ENDPOINT = process.env.TRIDENT_VLM_ENDPOINT || 'http://127.0.0.1:8082';
const VLM_MODEL = process.env.TRIDENT_VLM_MODEL || 'glm-4v-flash';
tridentLog('INFO', 'trident-vision', `Using VLM endpoint: ${VLM_ENDPOINT}`);
...
export const tridentVisionTool = tool({
  ...
  execute: async (args: {
    ...
    if (args.action === 'status') {
      try {
        const resp = await fetch(`${VLM_ENDPOINT}/health`, { signal: AbortSignal.timeout(3000) });
        if (resp.ok) {
      ...
      return JSON.stringify({
        ...
        tip: 'Ensure llama-server is running. Default: http://127.0.0.1:8082. Set TRIDENT_VLM_ENDPOINT to override.',
      }, null, 2);
    }
    ...
    try {
      ...
      const secureResult = await resolveSecurePath(imagePath, workspaceRoot);
      if (secureResult instanceof Error) {
        return JSON.stringify({ error: 'Path validation failed', message: secureResult.message, path: imagePath }, null, 2);
      }
      const resolved = secureResult;
      ...
      const response = await fetch(`${VLM_ENDPOINT}/v1/chat/completions`, {
        method: 'POST',
      ...
      return JSON.stringify({
        ...
        tip: errorType === 'VLM_CONNECTION_FAILED'
          ? `Cannot reach llama-server at ${VLM_ENDPOINT}. Ensure it's running: llama-server -m GLM-4.6V-Flash-Q4_K_M.gguf --port 8082`
          : undefined,
      }, null, 2);
    }
  },
