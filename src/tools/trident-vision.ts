 ...
 *   TRIDENT_VLM_HOST  - llama-server host (default: http://127.0.0.1:8082)
...
const VLM_BASE_URL = (process.env.TRIDENT_VLM_HOST || 'http://127.0.0.1:8082').replace(/\/+$/, '');
const VLM_MODEL = process.env.TRIDENT_VLM_MODEL || 'GLM-4.6V-Flash-Q4_K_M.gguf';
    ...
    try {
      ...
      return JSON.stringify({
        ...
        tip: errorType === 'VLM_CONNECTION_FAILED'
          ? `Cannot reach llama-server at ${VLM_BASE_URL}. Ensure it's running: llama-server -m GLM-4.6V-Flash-Q4_K_M.gguf --port 8082`
          : undefined,
      }, null, 2);
    }
  },
