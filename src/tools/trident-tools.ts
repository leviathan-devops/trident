...
import { validateDeepPlanningInput, validateContextSynthesisInput, validateProblemSolvingInput } from './input-validation.js';
    ...
    'trident-deep-planning': tool({
      ...
      execute: async (args: {
        ...
        try {
          ...
          const layer = typeof args.layer === 'string' ? parseInt(args.layer as string) || 1 : (args.layer || 1);
          ...
          try {
            ...
            const L2_SYSTEM = 'You are an elite engineering spec writer. ' +
              ...
              'Target 2000+ lines. NO MAXIMUM. 5000+ is normal for complex systems. ' +
            ...
            for (let l2Iter = 1; l2Iter <= MAX_L2_ITERATIONS; l2Iter++) {
              ...
              try {
                generatedSpec = await generateSpecViaLLM(l2Brief, lastFeedback || undefined, false, L2_SYSTEM, undefined, 2000);
              } catch (llmErr) {
              ...
              // ── LINE COUNT GATE ──
              // L2 specs must be 2000+ lines. If below, the LLM must AUDIT the
              // entire document for missing context/data and SURGICALLY EXPAND
              // deficient sections — not regenerate. Thin sections are identified
              // by heading and fed back as an explicit expansion work order.
              const L2_MIN_LINES = 2000;
              const currentLines = generatedSpec.split('\n').length;
              if (currentLines < L2_MIN_LINES) {
                tridentLog('WARN', 'trident-deep-planning',
                  `L2 LINE COUNT FAILURE: ${currentLines} lines < ${L2_MIN_LINES} minimum — requesting surgical expansion (iteration ${l2Iter}/${MAX_L2_ITERATIONS})`);
                ...
                lastFeedback =
                  `LINE COUNT FAILURE: Output is ${currentLines} lines. Minimum is ${L2_MIN_LINES} lines.\n\n` +
                  `DO NOT regenerate from scratch. AUDIT the ENTIRE document for missing context and data, then SURGICALLY EXPAND it.\n\n` +
                  `EXPANSION WORK ORDER:\n` +
                  `1. These sections are thin and MUST be expanded to full engineering depth:\n${thinList}\n\n` +
                  `2. Every section must contain: full pseudocode or interfaces, data flows, failure modes with root causes, design rationale with rejected alternatives, and cross-references to related sections.\n\n` +
                  `3. Use the provided context fields (components, constraints, designDecisions, knownGaps, sourceLineage, fileInventory) — they contain material you have NOT yet incorporated. Expand each component into its own deep-dive subsection.\n\n` +
                  `4. Keep all existing correct content UNCHANGED. Expand around it.\n\n` +
                  `Output the COMPLETE expanded document (all sections, original + expanded content).`;
                if (l2Iter === MAX_L2_ITERATIONS) {
                  tridentLog('ERROR', 'trident-deep-planning',
                    `L2 under ${L2_MIN_LINES} lines after ${MAX_L2_ITERATIONS} iterations — saving ${currentLines}-line output anyway`);
                }
                continue; // Next iteration with expansion feedback
              }

              // Topic aligned AND line count met — done
              break;
            }
...
📄 Artifact saved: ${l2ArtifactPath}`;
          ...
          try {
            ...
            if ((!l3Domains || l3Domains.length === 0) && args.domainsJson) {
              try {
                l3Domains = JSON.parse(args.domainsJson);
          ...
          // ═══ v4.4.2 T1 INTERNAL LLM GENERATION ═══
          {
            try {
              ...
              const CS_T1_SYSTEM = 'You are an elite context synthesizer. Generate a T1 injectable. ' +
                'Target 200+ lines. COMPACT but COMPLETE — every essential fact, decision, bug. ' +
