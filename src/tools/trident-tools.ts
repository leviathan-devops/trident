...
import { discoverProject, type DiscoveryResult } from '../shared/auto-discover.js';
...
function toolResultOk<T>(data: T, validator?: (data: T) => string | null): ToolResult<T> {
  if (validator) {
    const validationError = validator(data);
    if (validationError) {
      tridentLog('WARN', 'trident-tools', `toolResultOk validation failed: ${validationError}`);
      return { ok: false, error: validationError };
    }
  }
  if (data === null || data === undefined) {
    return { ok: false, error: 'toolResultOk received null/undefined data' };
  }
  return { ok: true, data };
}

function toolResultErr<T>(error: string): ToolResult<T> {
    ...
    'trident-deep-planning': tool({
      ...
      execute: async (args) => {
        ...
        try {
          ...
          try { orchestrator.startPlanning(); } catch { /* non-blocking */ }
          ...
          const requirements = args.requirements || (discovery
            ? `Auto-discovered project: ${discovery.totalFiles} files, ${discovery.totalLines} lines across ${Object.keys(discovery.languages).length} languages. Entry points: ${discovery.entryPoints.join(', ') || 'none detected'}. Languages: ${Object.entries(discovery.languages).map(([k, v]) => `${k} (${v} files)`).join(', ')}.`
          ...
          const discoveryPatterns = discovery
            ? discovery.patterns.map(p => `${p.name} (${p.type}) — ${p.file}:${p.line}`)
            : [];
          ...
          const discoveryFailures = discovery
            ? discovery.failureModes.map(f => `${f.message} — ${f.file}:${f.line} [pattern: ${f.pattern}]`)
            : [];
          ...
          const discoveryDecisions = discovery
            ? discovery.decisions.map(d => `${d.rationale} — ${d.file}:${d.line}`)
            : [];
