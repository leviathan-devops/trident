...
export const R15_CONTAINER_PREFLIGHT: LayerRule = {
  ...
  evaluate(construct: CodeConstruct | null, ctx: AnalysisContext): AuditFinding[] {
    ...
    // THE 2026-08-10 FIX: the closure `visit` cannot see the guard's narrowing
    // (TS control-flow analysis does not propagate into nested closures) — the
    // 12 TS18047s fired on c.filePath/line/type inside visit. The const
    // binding captures the narrowed non-null type at this scope.
    const c = construct;
