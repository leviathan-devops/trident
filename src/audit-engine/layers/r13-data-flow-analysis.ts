...
function buildDataFlowGraph(
  ...
  for (const construct of constructs) {
    ...
    const envSources = nodes.filter(n => n.sourceType === 'env' && n.filePath === construct.filePath);
    const jsonSources = nodes.filter(n => n.sourceType === 'json_parse' && n.filePath === construct.filePath);
    const sinks = nodes.filter(n => n.kind === 'sink' && n.filePath === construct.filePath);
...
export const R13_DATA_FLOW_ANALYSIS: LayerRule = {
  ...
  evaluate(construct: CodeConstruct | null, ctx: AnalysisContext): AuditFinding[] {
    ...
    try {
      ...
      checker = null;
      // Safe to continue — TypeChecker creation failed, analysis falls back to regex-based checks
    }
      ...
      while ((directMatch = directCallPattern.exec(body)) !== null) {
        ...
        if (hasEnvInput || hasParamInput) {
          findings.push({
            ...
            description: `Dangerous sink ${pattern.call}() called with potentially unvalidated input`,
            correction: `Validate and sanitize input before passing to ${pattern.call}()`,
    ...
    if (construct.node && checker) {
      try {
        const allConstructs = ctx.constructs.filter(
          c => c.filePath === construct.filePath && c.node
        );
          ...
          if (edge.from.kind === 'source' && edge.to.kind === 'sink') {
            if (edge.from.sourceType === 'env' || edge.from.sourceType === 'json_parse') {
              const alreadyReported = findings.some(
                f => f.file === edge.to.filePath && Math.abs(f.line - edge.to.line) <= 2 && f.category === 'DATA_FLOW'
              );
              if (!alreadyReported) {
                findings.push({
                  layer: 'R13',
        ...
        // TypeChecker not available — regex-based analysis above is sufficient
      }
    }

    const anyTypeParams = construct.parameters.filter(p => p.type === 'any' || p.type === null);
