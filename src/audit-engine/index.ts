...
export class AuditEngine {
  ...
  async audit(targetPath: string): Promise<AuditResult> {
    ...
    if (srcFilesScanned === 0) {
      ...
      result.report = this.generateReport(result, targetPath, pkgJson?.name || path.basename(targetPath), 'trident');
      return result;
    }
    ...
    const projectName = pkgJson?.name || path.basename(targetPath);
    const agentName = opencodeJson?.agent ? Object.keys(opencodeJson.agent)[0] || 'trident' : 'trident';
