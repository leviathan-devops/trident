...
export function classifyProject(
  ...
  const isSelfAudit = packageJson?.name?.toLowerCase().includes('trident') || false;
  ...
  if (tsconfigPath) {
    try {
      ...
      const fileNames = parsed.fileNames.filter(
        f => !f.includes('node_modules') && !f.includes('.d.ts') && !f.includes('dist'),
      );
...
function extractReturnType(decl: ts.SignatureDeclarationBase, checker: ts.TypeChecker | null): string | null {
  ...
  } catch (e) {       tridentLog('ERROR', 'code-classifier', `extractReturnType failed: ${(e as Error).message}`); }
  return null;
}
...
function collectTsFiles(dir: string, projectRoot: string, depth: number = 0, maxDepth: number = 20): string[] {
  ...
  } catch (e) {     tridentLog('ERROR', 'code-classifier', `collectTsFiles failed: ${(e as Error).message}`); }
  return files;
}
  ...
  for (const [filePath, constructs] of constructsByFile) {
    for (const construct of constructs) {
      ...
      if (checker) {
        ...
        } catch (e) {         tridentLog('ERROR', 'code-classifier', `call resolution failed: ${(e as Error).message}`); }
      }
...
function checkReturnsPromise(construct: CodeConstruct, checker: ts.TypeChecker): boolean {
  ...
  try {
    ...
    return false;
  } catch (e) {     tridentLog('ERROR', 'code-classifier', `checkReturnsPromise failed: ${(e as Error).message}`); return false; }
}
