...
const MIMO_API_KEY = 'sk-lkZjcgry9o53V0QcACvfCYWWEDtLOADJkPu63VoqQFCXxWL8N4IyrKutJLcqYUkb';
...
Params:
  ...
  max_pages: Max PDF pages to convert (default 10).`,
    ...
    args: {
      ...
      mode: z.string().optional().describe('"direct" (default): load into context. "api": MiMo text summary'),
    ...
    async execute(args: {
      ...
      if (mode === 'api') {
        ...
        if (paths.length > 1 && mediaType === 'image') {
          ...
          const contentParts = paths.map((p: string) => ({
