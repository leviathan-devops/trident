...
### Diagram A — Trident Agent (Operator Driven w/ Autonomous Phases)
...
THE REAL TRIDENT BUILD WORKFLOW — MACRO ENGINEERING LOOP
   ...
   SESSION START
        ...
        ▼
   ┌──────────────────────────────┐
   │ PHASE 1 · PLANNING LOOP      │  ← anchors the entire build
   └────────────┬─────────────────┘
        │
        │  PRIMARY: CONTEXT CREATION — the detailed
        │  planning workflow that writes the canon
        │  docs / bible / specs the build runs against:
        │  BUILD_STATE · DECISION_CHAIN · OVERHAUL_SPEC ·
        │  COMPACTION_SURVIVAL · canon context docs
        │
        │  SECONDARY: CONTEXT RECOVERY — only on loops
        │  or post-compaction:
        │  explore subagents (5×) · hive_context ·
        │  session-log recovery · checkpoint read
        │
        ▼
   ┌──────────────────────────────┐
   │ PHASE 2 · BUILD              │  ← subagent waves execute
   └────────────┬─────────────────┘
        │
        │  dispatch waves per the spec:
        │  ├─ trident_build    ×N files (one agent per
  ...
  (surgical,        │
   ...
   └────────────┬─────────────────┘
        │
        │  fixes applied — operator edits or
        │  trident_build wave re-dispatch
  ...
  (8/8 toy ·       fix loop (PHASE 4)
   ...
   └────────────┬─────────────────┘
        ...
        │  dist + src + docs + BUILD_REPORT + DEBUG_LOG
        │
        ▼
   ┌──────────────────────────────┐
   │ PHASE 8 · SHIP               │
