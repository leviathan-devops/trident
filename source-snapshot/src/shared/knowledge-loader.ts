...
function logMessage(level: string, msg: string): void {
  try {
    ...
    // Last-resort fallback: no-op if console is unavailable
  }
}
...
export function loadKnowledgeLibrary(kbId: string): KnowledgeContent {
  try {
    ...
    if (!filename) {
      ...
      try { _knowledgeCache.set(kbId, result); } catch (_e: unknown) { /* cache best-effort */ }
      return result;
    }
...
export function invalidateKnowledgeCache(): void {
  try { _knowledgeCache.clear(); logMessage('INFO', 'Knowledge cache invalidated'); } catch (_e: unknown) { /* best-effort */ }
}

export function getCachedKnowledgeCount(): number {
  try { return _knowledgeCache.size; } catch (_e: unknown) { return 0; }
}
  ...
  try {
    ...
    if (!found) {
      return {
        ...
        error: `Technique ${techniqueNumber} not found in ${kbId}`,
      };
    }
    return {
      id: `${kbId}-t${techniqueNumber}`, filename: full.filename,
