...
function log(level: string, component: string, message: string): void {
  try {
    ...
    console.error(`[paths] ${level}: ${message}`, e instanceof Error ? e.message : String(e));
  }
}
...
function loadAgentDirMap(): Record<string, string> {
  ...
  try {
    ...
    console.error('[paths] Agent config load failed:', e instanceof Error ? e.message : String(e));
...
function resolveWorkspaceRoot(): string {
  ...
  try {
    ...
    console.error('[paths] Script dir walk failed:', e instanceof Error ? e.message : String(e));
