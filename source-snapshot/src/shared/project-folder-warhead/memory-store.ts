...
function log(level: string, component: string, message: string): void {
  try {
    ...
    console.error('[memory-store] Log write failed:', e instanceof Error ? e.message : String(e));
  }
}
...
try {
  ...
  console.error('[memory-store] No valid marker found:', errMsg);
...
export function persistMarkerFile(): void {
  ...
  try {
    ...
    console.error('[memory-store] Failed to persist marker file:', errMsg);
