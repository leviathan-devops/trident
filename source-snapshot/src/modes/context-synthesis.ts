...
**SOURCES:** T1 (Session), T2 (Knowledge), T3 (Files), T4 (Tools)`;
  ...
  buildExplorerDispatchTemplate(targetPaths: string[], count: number): string {
    // [P2] Validate inputs
    if (!Array.isArray(targetPaths) || targetPaths.length === 0) {
      return '[EXPLORER DISPATCH ERROR] No target paths provided.';
    }
    if (typeof count !== 'number' || count < 1) {
      count = 1;
    }
    return [
      '# TRIDENT EXPLORER DISPATCH — DEPLOY ' + count + ' EXPLORERS',
      '## Targets (' + targetPaths.length + ' files)',
      ...targetPaths.map(function(f: string, i: number) { return (i + 1) + '. ' + f; }),
      '## Protocol',
      '1. task description="Read <target>, return V2 synthesis" subagent_type=trident_explore',
      '2. WAIT for ALL explorers to return V2 reports',
      '3. Merge into single V2 synthesis',
      '4. Feed into warhead synthesizer for T1 generation',
      '## Constraints',
      '- Explorer tools: read, glob, grep, hive_context ONLY',
      '- Explorer blocked: write, edit, bash, terminal, execute, task',
      '- Each explorer: 10-20 min timeout',
      '- Zero resistance to deployment count (L5.11 enforced)',
    ].join('\n');
  }
}
