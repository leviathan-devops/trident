...
export class StreamingIntentParser {
  ...
  private analyzeSentence(text: string): StreamingIntentResult {
    ...
    const tokens: string[] = doc.tokens().out() as unknown as string[];
    const posTags: string[] = doc.tokens().out((t: unknown) => (t as { pos: () => string }).pos()) as unknown as string[];
    ...
    try {
      ...
      console.error('[streaming-buffer] NER unavailable:', e instanceof Error ? e.message : String(e));
    }
...
const streamingParser = new StreamingIntentParser();
