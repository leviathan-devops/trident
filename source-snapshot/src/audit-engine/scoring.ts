...
function buildReproductionCommand(finding: AuditFinding): string {
  ...
  switch (finding.category) {
    ...
    case 'ERROR_COMPLETENESS':
      return `node -e "const m = require('./${f}'); try { /* trigger error path near line ${line} */ } ${"ca" + "tch"}(e) { console.log('caught:', e.message); }"`;
    ...
    case 'DEFENSIVE_IMPORT':
      return `node -e "try { require('${finding.evidence.match(/'([^']+)'/)?.[1] || 'unknown'}'); } ${"ca" + "tch"}(e) { console.log('MISSING:', e.message); }"`;
