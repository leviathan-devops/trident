...
export const R2_STATE_MACHINE: LayerRule = {
  ...
  evaluate(construct: CodeConstruct | null, ctx: AnalysisContext): AuditFinding[] {
    ...
    if (!body.includes('advanceLayer') && !/\bCOMPLETE\b/.test(body) && !body.includes('layer')) return findings;

    const hasAdvanceLayer = body.includes('advanceLayer');
    const hasComplete = /\bCOMPLETE\b/.test(body);
...
function extractReturnPathsAST(construct: CodeConstruct): ReturnPathAST[] {
  ...
  function walk(n: ts.Node, pathHasAdvanceLayer: boolean): void {
    ...
    if (ts.isReturnStatement(n)) {
      ...
      const includesComplete = /\bCOMPLETE\b/.test(returnText);
      paths.push({
    ...
    if (ts.isCallExpression(n)) {
      ...
      if (calleeText.includes('advanceLayer')) {
        ts.forEachChild(n, (child: ts.Node) => walk(child, true));
        return;
      }
    }

    ts.forEachChild(n, (child: ts.Node) => walk(child, pathHasAdvanceLayer));
  }

  ts.forEachChild(node.body, (child: ts.Node) => walk(child, false));
  return paths;
}
