...
export const R4_ERROR_HANDLING: LayerRule = {
  ...
  evaluate(construct: CodeConstruct | null, ctx: AnalysisContext): AuditFinding[] {
    if (!construct) return [];
    // Line number sanity check: skip if line exceeds source file length (AST position bug)
    if (construct.line > 0 && construct.filePath && ctx.sourceFiles) {
      const srcFile = ctx.sourceFiles.get(construct.filePath);
      if (srcFile) {
        const maxLine = srcFile.text.split('\n').length;
        if (construct.line > maxLine) return [];
      }
    }
    ...
    const hasLogging = bodyLower.includes('console.error') || bodyLower.includes('console.warn') || bodyLower.includes('console.log');
    ...
    if (body.includes('fall back') || body.includes('fallback') || body.includes('/* fall back')) {
      const hasLog = body.toLowerCase().includes('console.error') || body.toLowerCase().includes('console.warn');
      if (!hasLog) {
        findings.push({
          layer: 'R4',
          severity: 'MEDIUM',
