...
export async function loadHivePatterns(docsDir: string): Promise<KnownPattern[]> {
  ...
  try {
    ...
    console.error('[hive-loader] Failed to load hive patterns:', e instanceof Error ? e.message : String(e));
  ...
  return findings.map((finding: AuditFinding) => {
    for (const pattern of patterns) {
      ...
      try {
        ...
        console.error('[hive-loader] Pattern match failed:', e instanceof Error ? e.message : String(e));
...
function parseMetaPatterns(content: string): KnownPattern[] {
  ...
  const sections: string[] = content.split(/### CATEGORY \d+: /);
