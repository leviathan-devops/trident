...
import { AuditResult, AuditFinding, ConfidenceDistribution, LayerRule } from './types.ts';
...
export class AuditEngine {
  ...
  private buildEmptyResult(targetPath: string, preflight: PreflightResult, pkgJson: Record<string, unknown> | null): AuditResult {
  ...
  async audit(targetPath: string): Promise<AuditResult> {
    ...
    const srcFilesScanned = Array.from(ctx.constructsByFile.keys()).filter(
      (f: string) => f.endsWith('.ts') && !f.includes('node_modules')
    ).length;

    if (srcFilesScanned === 0) {
      const result = this.buildEmptyResult(targetPath, preflight, pkgJson);
      result.report = this.generateReport(result, targetPath, String(pkgJson?.name || path.basename(targetPath)), 'trident');
      return result;
    }
    ...
    const projectName = String(pkgJson?.name || path.basename(targetPath));
    const agentConfig = opencodeJson?.agent;
      const agentName = agentConfig && typeof agentConfig === 'object'
        ? String(Object.keys(agentConfig as Record<string, unknown>)[0] || 'trident')
        : 'trident';
  ...
  private computeLayerStats(findings: AuditFinding[]): { layer: string; name: string; findingCount: number; avgConfidence: number; evidenceSuppressed: boolean }[] {
    ...
    const allLayers = Object.entries(layerNames).map(([layer, name]: [string, string]) => {
  ...
  private async readJson(filePath: string): Promise<Record<string, unknown> | null> {
    try {
      ...
      tridentLog('WARN', 'audit-engine', 'readJson failed for ' + filePath + ': ' + (e instanceof Error ? e.message : String(e)));
