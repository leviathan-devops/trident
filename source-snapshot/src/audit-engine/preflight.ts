...
async function fileExists(filePath: string): Promise<boolean> {
  try {
    ...
    return false; // file not found — expected for some paths
  }
}
...
export async function runPreflight(targetPath: string): Promise<PreflightResult> {
  ...
  try {
    if (await fileExists(pkgPath)) {
      ...
      const parsedRaw = JSON.parse(raw); const parsedPkg = parsedRaw && typeof parsedRaw === 'object' ? parsedRaw as Record<string, unknown> : null;
      if (parsedPkg && typeof parsedPkg === 'object') {
        pkg = parsedPkg as Record<string, unknown>;
      }
    }
  } catch (e: unknown) {
    tridentLog('WARN', 'preflight', 'Failed to parse package.json: ' + (e instanceof Error ? e.message : String(e)));
    tridentLog('WARN', 'preflight', `Failed to parse package.json: ${(e instanceof Error ? e : new Error(String(e))).message}`);
    return {
      typeCheckPassed: false,
      typeCheckError: 'Failed to parse package.json: ' + ((e instanceof Error ? e : new Error(String(e))).message || String(e)),
      buildPassed: false,
      buildError: null,
      distExists: false,
      ...
      findings: [{ check: 'preflight', passed: false, detail: 'Failed to parse package.json: ' + ((e instanceof Error ? e : new Error(String(e))).message || String(e)) }],
    };
  }
  ...
  const pkgObj = (pkg && typeof pkg === 'object') ? (pkg as Record<string, unknown>) : undefined;
  const pkgScripts = pkgObj?.scripts as Record<string, unknown> | undefined;
  if (pkgScripts?.['build:check']) {
    try {
      ...
      tridentLog('WARN', 'preflight', 'Type check failed: ' + (e instanceof Error ? e.message : String(e)));
  ...
  if (pkgScripts?.build) {
    try {
      ...
      tridentLog('WARN', 'preflight', 'Build failed: ' + (e instanceof Error ? e.message : String(e)));
  ...
  const sourceMapExists = await fileExists(path.join(targetPath, 'dist', 'index.js.map'));
  ...
  if (distExists) {
    ...
    const distEntries = await fs.readdir(path.join(targetPath, 'dist'));
    const otherDistFiles = distEntries
      .filter((f: string) => f !== 'index.js' && f !== 'index.js.map' && f.endsWith('.js'));
